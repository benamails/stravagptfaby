...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!code && !refresh_token) return NextResponse.json({ ok: false, error: "Missing code or refresh_token" }, { status: 400 });
    ...
    if (athlete_id) await saveTokens(athlete_id, mapped);

    logger.info("[/oauth/token] success", { reqId, athlete_id: mapped.athlete_id, expires_at: mapped.expires_at, t: `${Date.now() - t0}ms` });
