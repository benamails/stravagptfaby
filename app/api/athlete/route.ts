// app/api/athlete/route.ts
// Retourne le profil athlète Strava lié à un user_id donné.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Filtrage minimal pour éviter de renvoyer des données inutiles
    const athlete = {
      id: raw.id,
      ...
      country: raw.country,
      city: raw.city,
      profile: raw.profile, // photo
      updated_at: raw.updated_at,
    };

    logger.info("[athlete] success", {
      reqId,
      userId,
      athleteId,
      t: `${Date.now() - t0}ms`,
