...
export interface StravaTokenResponse {
  ...
  athlete: { id: number; username?: string; firstname?: string; lastname?: string; [k: string]: any; };
  scope?: string;
}
...
export async function fetchActivity(accessToken: string, id: string) {
  const res = await fetch(`${STRAVA_API_BASE}/activities/${id}`, { headers: { Authorization: `Bearer ${accessToken}` } });
  if (!res.ok) {
    const text = await res.text();
...
export async function fetchAthlete(accessToken: string) {
  const res = await fetch(`${STRAVA_API_BASE}/athlete`, { headers: { Authorization: `Bearer ${accessToken}` } });
  if (!res.ok) {
    const text = await res.text();
